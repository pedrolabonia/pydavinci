...
class TimelineSettings(BaseConfig):
    ...
    output_resolution_height: conint(ge=1) = Field(alias="timelineOutputResolutionHeight")
    output_resolution_width: conint(ge=1) = Field(alias="timelineOutputResolutionWidth")
    pixel_aspect_ratio: Literal["square", "cinemascope"] = Field(alias="timelinePixelAspectRatio")
    resolution_height: conint(ge=1) = Field(alias="timelineResolutionHeight")
    resolution_width: conint(ge=1) = Field(alias="timelineResolutionWidth")
...
class CommonSettings(BaseConfig):
    ...
    def superscale_validator(cls, value, field): # TODO: ta invertido aqui os paranaue
        return super_scale_reverse(value)
...
data = resolve.project.get_setting(None)

audio = Audio.parse_obj(data)
color = Color.parse_obj(data)
perf = Perf.parse_obj(data)
deck = Deck.parse_obj(data)
capture = Capture.parse_obj(data)
playout = Playout.parse_obj(data)
timeline = TimelineSettings.parse_obj(data)
_projectsettings = _ProjectSettings.parse_obj(data).dict()

const = {"audio": audio, "color": color, "perf": perf, "deck": deck, "capture": capture, "playout": playout, "timeline": timeline, **_projectsettings}


def get_prj_settings():
